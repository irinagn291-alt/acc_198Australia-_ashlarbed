...
- Invariant (unit-test this): tonnage = weight×reps. Epley 1RM = w×(1+reps/30). 500 kg = 1 floor. Daily floor goal from experience × aim.
- Empty: The tower has no floors yet. Log the first set.
- Fake that fails: A workout table with no tonnage→floor mapping.
...
**Mini-ref `Rise`** (`lift_tower`)
- Steal: Stack that grows from logged work, empty today, peak height.
- Never: Not a crash-tower game. No wrapper.
